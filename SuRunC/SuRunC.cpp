...
int wmain(int argc, wchar_t *argv[], wchar_t *envp[]) {
  ...
  STARTUPINFO si = {0};
  si.cb = sizeof(si);
  DWORD MaxLen = GetSystemWindowsDirectory(0, 0) +
                 (DWORD)_tcslen(GetCommandLine()) + MAX_PATH;
  ...
  if (CreateProcess(NULL, cmd, NULL, NULL, FALSE, CREATE_SUSPENDED, NULL, NULL,
    ...
    PathQuoteSpaces(cmd);
    _stprintf(&cmd[_tcslen(cmd)], L" /CONSPID %d %s", pi.dwProcessId,
    ...
    if (CreateProcess(NULL, cmd, NULL, NULL, FALSE, CREATE_SUSPENDED, NULL,
      ...
      if (p) {
        ...
        TRACEx(L"OpenProcess(%d) failed: %s", pi.dwProcessId,
