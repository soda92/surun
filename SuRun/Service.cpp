...
DWORD CheckCliProcess(RUNDATA& rd)
{
  ...
  if (sizeof(RUNDATA)!=s)
  {
    DBGTrace2("ReadProcessMemory wrong size. Expected %d, got %d",sizeof(RUNDATA),s);
    return 0;
  }
...
VOID WINAPI ServiceMain(DWORD argc,LPTSTR *argv)
{
  ...
  g_ss.dwWaitHint         = 0;
  SetServiceStatus(g_hSS,&g_ss);
  DWORD StartDelay=max(0,min(600,GetRegInt(HKLM,SURUNKEY,L"StartDelay",0)));
...
//           }
          ...
          if (g_RunData.bShlExHook)
          {
            ...
            {
              ResumeClient(RETVAL_SX_NOTINLIST);
              DBGTrace2("User is no SuRunner: %s: %s",g_RunData.UserName,g_RunData.cmdLine);
              continue;
            }
...
TryAgain:
                                    ...
                                    0,NULL,&si,&pi))
            {
              bRunCount++;
              SIZE_T N=0;
              if (!WriteProcessMemory(pi.hProcess,BASE_ADDR(&g_RunData,pi.hProcess),&rd,sizeof(rd),&N))
                DBGTrace1("WriteProcessMemory failed %s", GetLastErrorNameStatic());

              if (!WriteProcessMemory(pi.hProcess,BASE_ADDR(&g_CliIsAdmin,pi.hProcess),&g_CliIsAdmin,sizeof(g_CliIsAdmin),&N))
                DBGTrace1("WriteProcessMemory failed %s", GetLastErrorNameStatic());
              //if (!g_RunData.bRunAs)
              {
                HANDLE hTok=0;
                ...
                if (!WriteProcessMemory(pi.hProcess,BASE_ADDR(&g_AdminTStat,pi.hProcess),&tstat,sizeof(TOKEN_STATISTICS),&N))
                  DBGTrace1("WriteProcessMemory failed %s", GetLastErrorNameStatic());
              }
              ResumeThread(pi.hThread);
              CloseHandle(pi.hThread);
