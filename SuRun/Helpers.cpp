...
BOOL EnablePrivilege(LPCTSTR name)
{
  ...
  if(!OpenThreadToken(GetCurrentThread(),TOKEN_ADJUST_PRIVILEGES,0,&hToken))
  {
    ...
    if (!bRet)
    {
      DBGTrace1("EnablePrivilege(%s) OpenThreadToken(GetCurrentThread()) failed", name);
    }
  }
  else
  {
  ...
  {
    {
      DBGTrace1("EnablePrivilege(%s) OpenProcessToken(GetCurrentProcess()) failed", name);
    }
    return FALSE;
  }
  bRet&=EnablePrivilege(hToken,name);
...
{
  EnablePrivilege(SE_DEBUG_NAME);
  HANDLE hProc = OpenProcess(SYNCHRONIZE|PROCESS_QUERY_LIMITED_INFORMATION,TRUE,ProcId);
  if (!hProc)
  {
  ...
  // Open impersonation token for process
  HANDLE hToken=NULL;
  CHK_BOOL_FN(OpenProcessToken(hProc,TOKEN_IMPERSONATE|TOKEN_QUERY|TOKEN_DUPLICATE|TOKEN_ASSIGN_PRIMARY,&hToken));
  CloseHandle(hProc);
  if(!hToken)
  {
